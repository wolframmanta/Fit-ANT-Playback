...
- `fit_ant_playback_qt.py`: DMRL-branded PySide6 dashboard entry point.
- `fit_ant_playback_core/fit_parser.py`: FIT file parsing.
- `fit_ant_playback_core/workout_parser.py`: structured workout file parsing.
...
- Workout parsing issue: start in `fit_ant_playback_core/workout_parser.py` and `tests/test_workout_parser.py`.
- Ride simulator issue: start in `fit_ant_playback_core/ride_simulator.py` and `tests/test_ride_simulator.py`.
...
- Run Qt app locally, only when intended: `python3 fit_ant_playback_qt.py`
